...
def rule_based_commentary(facts):
    ...
    if eff:
        ...
        if var is not None and var < 0 and neg: why += f" The shortfall is concentrated in {neg[0][0]} ({_fmt_m(neg[0][1])})."
        elif var is not None and var >= 0 and pos: why += f" Growth is led by {pos[0][0]} ({_fmt_m(pos[0][1])})."
    gm_move = gm.get("variance")
    if gm_move is not None and not (isinstance(gm_move, float) and np.isnan(gm_move)): why += f" GM% moved {gm_move*100:+.1f}pp vs budget."
...
def commentary(df, base="budget"):
    facts = build_facts(df, base); rb = rule_based_commentary(facts)
    provider = get_provider()
    if not provider.available(): return {**rb, "facts": facts}
    system = ("You are a commercial finance director. Given ONLY calculated figures in JSON incl. a "
              "sales attribution. NEVER invent numbers; quote exactly and reference the biggest effects. "
              "Three labelled parts: WHAT, WHY, SO WHAT. 4-6 sentences.")
...
def answer_question(df, question, base="budget"):
    ...
    if not provider.available():
        rb = rule_based_commentary(facts); return {"answer": f"{rb['what']} {rb['why']}", "source": "rule-based", "facts": facts}
    system = ("You are an FP&A analyst. Answer using ONLY the JSON facts (pre-calculated). Never invent "
              "numbers, quote exactly, cite effects/members, end with 'Management focus:' when relevant. 2-4 sentences.")
    try:
        ...
        rb = rule_based_commentary(facts); return {"answer": f"{rb['what']} {rb['why']}", "source": "rule-based", "facts": facts, "llm_error": str(e)}
...
def _slim_facts(facts):
    return {"kpis": _slim(facts["kpis"]), "attribution": facts["attribution"], "top_customers": facts["top_customers"], "top_products": facts["top_products"], "market": facts["market"]}
