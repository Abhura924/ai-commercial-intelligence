...
def _layout(fig, title=None, height=340):
    fig.update_layout(title=dict(text=title or "", font=dict(family="Source Serif 4", size=16, color=INK)),
        height=height, plot_bgcolor="white", paper_bgcolor="white", margin=dict(t=44 if title else 14, b=28, l=12, r=14),
        font=dict(family="IBM Plex Sans", color=INK, size=12), legend=dict(orientation="h", y=-0.2, x=0, font=dict(size=12)))
...
def sales_trend(ts):
    ...
    if "budget" in ts and ts["budget"].notna().any(): fig.add_trace(go.Scatter(x=ts["period"], y=ts["budget"], name="Budget", line=dict(color=AMBER, width=2, dash="dash")))
    if "prior_year" in ts and ts["prior_year"].notna().any(): fig.add_trace(go.Scatter(x=ts["period"], y=ts["prior_year"], name="Prior year", line=dict(color="#9CA0A8", width=1.75)))
    fig.update_yaxes(tickprefix="£", tickformat=".2s"); return _layout(fig)

def gm_trend(ts):
    fig = go.Figure()
    if "gm_pct" in ts: fig.add_trace(go.Scatter(x=ts["period"], y=ts["gm_pct"], name="Actual GM%", mode="lines+markers", line=dict(color=TEAL, width=3), marker=dict(size=5)))
    if "budget_gm_pct" in ts and ts["budget_gm_pct"].notna().any(): fig.add_trace(go.Scatter(x=ts["period"], y=ts["budget_gm_pct"], name="Budget GM%", line=dict(color=AMBER, width=2, dash="dash")))
    if "py_gm_pct" in ts and ts["py_gm_pct"].notna().any(): fig.add_trace(go.Scatter(x=ts["period"], y=ts["py_gm_pct"], name="Prior year GM%", line=dict(color="#9CA0A8", width=1.75)))
    fig.update_yaxes(tickformat=".0%"); return _layout(fig)
...
def waterfall(bridge, unit="m"):
    ...
    fig = go.Figure(go.Waterfall(orientation="v", measure=measures, x=labels, y=values, text=text, textposition="outside",
        textfont=dict(size=11, color=INK), connector={"line": {"color": "#DEDCD2", "dash": "dot"}},
...
def scenario_chart(bundle):
    ...
    for scen, f in bundle.items():
        if f.get("ok"): fig.add_trace(go.Scatter(x=f["periods"], y=f["values"], name=scen, mode="lines+markers", line=dict(color=colors[scen], width=2, dash="dash"), marker=dict(size=4)))
    fig.update_yaxes(tickprefix="£", tickformat=".2s"); return _layout(fig, height=360)
...
def budget_phasing(q_budget, last_year):
    ...
    fig.update_yaxes(tickprefix="£", tickformat=".2s"); return _layout(fig, height=340)
...
def driver_bars(ranking):
    ...
    fig.update_xaxes(tickprefix="£", tickformat=".2s"); return _layout(fig, height=max(260, 40 * len(names)))

def effect_drill_bars(df, effect, by):
    if df is None or df.empty: return _layout(go.Figure(), height=200)
    ...
    fig.update_xaxes(tickprefix="£", tickformat=".2s"); return _layout(fig, height=max(220, 38 * len(d)))

def scenario_compare_bars(baseline, scenario, lbl_a="Current", lbl_b="Scenario"):
    metrics = ["sales", "gross_margin", "ebitda"]; labels = ["Sales", "Gross Margin", "EBITDA"]
    fig = go.Figure()
    fig.add_trace(go.Bar(x=labels, y=[baseline[m] for m in metrics], name=lbl_a, marker_color=MUTED, text=[fmt_m(baseline[m]) for m in metrics], textposition="outside"))
    fig.add_trace(go.Bar(x=labels, y=[scenario[m] for m in metrics], name=lbl_b, marker_color=TEAL, text=[fmt_m(scenario[m]) for m in metrics], textposition="outside"))
    fig.update_yaxes(tickprefix="£", tickformat=".2s"); fig.update_layout(barmode="group"); return _layout(fig, height=340)
