...
def process(raw, mapping_override=None, filename="upload", company="demo-co", source="excel"):
    ...
    canonical = cm.to_canonical(raw, mapping); findings = dq.run_checks(raw, canonical, mapping); summary = cm.canonical_summary(canonical)
    ds_id = store.new_id()
    try:
        store.save_raw(ds_id, raw, filename); store.save_processed(ds_id, canonical)
        store.save_metadata(DatasetRecord(dataset_id=ds_id, filename=filename, uploaded_at=ds_id.split("-")[0], company=company, source=source, period_min=summary.get("date_min"), period_max=summary.get("date_max"), mapping=mapping, quality=dq.summarise(findings), status="processed"))
    except Exception: pass
    return {"dataset_id": ds_id, "mapping": mapping, "canonical": canonical, "findings": findings, "summary": summary, "dictionary": sm.data_dictionary(raw, mapping), "derivations": sm.derivation_notes(mapping)}
