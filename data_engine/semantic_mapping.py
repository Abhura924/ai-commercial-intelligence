...
import numpy as np
import pandas as pd
SYNONYMS = {"date":["date","month","period","posting date","fiscal","week","quarter","yr","year"],
...
def derivation_notes(mapping):
    n=[]
    if "gross_margin" not in mapping and "revenue" in mapping and "cost" in mapping: n.append("Gross Margin not supplied — will be derived as Revenue − COGS.")
    if "price" not in mapping and "revenue" in mapping and "volume" in mapping: n.append("Price/ASP not supplied — will be derived as Revenue ÷ Volume.")
    if "budget_revenue" not in mapping: n.append("No Budget column detected — budget comparatives unavailable.")
    if "py_revenue" not in mapping: n.append("No Prior-Year column detected — YoY comparatives unavailable.")
    if "market_growth" not in mapping: n.append("No Market/Industry growth detected — market-share analysis limited.")
    return n
def data_dictionary(df,mapping):
    rows=[]; inv={v:k for k,v in mapping.items()}
    for col in df.columns:
        rows.append({"Source column":col,"Mapped to":inv.get(col,"(unmapped)"),"Dtype":str(df[col].dtype),"Sample values":", ".join(map(lambda x:str(x)[:18],df[col].dropna().head(3).tolist()))})
