"""Deterministic Calculation Engine — every financial number originates here."""
...
def variance(a, b):
    return np.nan if (a is None or b is None or np.isnan(a) or np.isnan(b)) else a - b


def variance_pct(a, b):
    return np.nan if (a is None or b is None or np.isnan(a) or np.isnan(b) or b == 0) else (a - b) / b
...
def kpi_block(df):
    out = {}
    for key, val in {"sales": sales(df), "gross_margin": gross_margin(df),
                     "gm_pct": gm_pct(df), "ebitda": ebitda(df)}.items():
        b, py = _budget(df, key), _prior_year(df, key)
        if key == "gm_pct":
            out[key] = {"value": val, "budget": b, "py": py, "variance": variance(val, b),
                        "variance_pct": variance(val, b), "py_pct": variance(val, py), "is_ratio": True}
        else:
            out[key] = {"value": val, "budget": b, "py": py, "variance": variance(val, b),
                        "variance_pct": variance_pct(val, b), "py_pct": variance_pct(val, py), "is_ratio": False}
    out["sales_vs_budget"] = {"value": out["sales"]["variance"], "budget": None, "py": None,
                              "variance": out["sales"]["variance"], "variance_pct": out["sales"]["variance_pct"],
                              "py_pct": None, "is_ratio": False}
    return out
