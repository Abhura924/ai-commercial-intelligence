"""Data Quality Engine — surfaces issues before analysis."""
...
def run_checks(raw, canonical, mapping):
    ...
    return f
def summarise(f):
    return {"ok":sum(1 for x in f if x["status"]=="ok"),"warn":sum(1 for x in f if x["status"]=="warn"),"error":sum(1 for x in f if x["status"]=="error")}
